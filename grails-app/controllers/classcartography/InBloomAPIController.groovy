...
class InBloomAPIController {
    ...
    def auth () {
    	...
    	def fullurl = "http://api.sandbox.inbloom.org/api/oauth/authorize?client_id=UzMIy7jMGB&response_type=code&redirect_uri=http://localhost:8080/cartography/login"
  	...
  	def saveSession () {
  	    ...
  	    withHttp(uri: "https://api.sandbox.inbloom.org") {
           tokenResp = get(path : '/api/oauth/token', query : [client_id:'UzMIy7jMGB', client_secret:'SN5oikX6ZhsE8h0RfQKqFR1DBW9eehNrag7jq4qL1VGzl6Bx', code:params.code, redirect_uri:'http://localhost:8080/cartography/'])
        }       
